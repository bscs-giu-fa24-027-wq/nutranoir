...
const initializeEmailService = () => {
  if (!process.env.GMAIL_USER || !process.env.GMAIL_PASSWORD) {
    console.warn('⚠️ Gmail credentials not configured. Email notifications will be skipped.');
    return null;
  }
...
const sendAdminOrderEmail = async (order, customerName, customerEmail) => {
  ...
  try {
    ...
    const htmlContent = `
      <div style="font-family: Arial, sans-serif; background: #f9f9f9; padding: 20px;">
        ...
        <p style="margin-top: 30px;">
          <a href="http://localhost:3000/admin.html" style="background: #d4af37; color: #1a1a1a; padding: 10px 20px; text-decoration: none; border-radius: 5px; font-weight: bold;">View in Admin Dashboard</a>
        </p>
      </div>
    `;
    ...
    console.log(`✅ Admin email sent to ${process.env.CUSTOMER_EMAIL}`);
    return true;
  } catch (error) {
...
const sendOrderStatusEmail = async (order, customerEmail, customerName) => {
  ...
  try {
    const statusMessages = {
      ...
      shipped: 'Your order has been shipped! Check tracking details below.',
      ...
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <div style="padding: 30px; background: #f9f9f9; border-radius: 0 0 5px 5px;">
          ...
          <p style="margin-top: 30px; color: #666; font-size: 12px;">
            If you have any questions, please contact us at support@nutranoir.com
          </p>
        </div>
      </div>
    `;
